...
var actorSystem = ActorSystem.Create("stor", akkaConfig);
...
builder.Services.AddHealthChecks().AddTypeActivatedCheck<ClusterHealthCheck>("akka", HealthStatus.Degraded);
...
{
    ResponseWriter = (context, result) => context.Response.WriteAsync(JsonSerializer.Serialize(new
        {
            akka = result.Entries["akka"].Data
        },
